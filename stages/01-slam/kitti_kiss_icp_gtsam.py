...
Usage
-----
  pip install kiss-icp gtsam numpy tqdm --break-system-packages
...
def run_kiss_icp(dataset: KITTIRawDataset) -> list[np.ndarray]:
    """Run KISS-ICP over every scan in the dataset, return world poses (4x4)."""
...
def main():
    ...
    ap.add_argument(
        ...
        default="cam0",
        help=(
            "coordinate frame for the written poses. 'cam0' matches the official "
            "KITTI odometry benchmark ground-truth convention (left camera frame) "
            "so results are directly comparable to poses.txt / published numbers. "
            "'lidar' keeps everything in the Velodyne frame KISS-ICP natively works in."
        ),
    )
    args = ap.parse_args()
    ...
    t0 = time.time()
    odom_poses = run_kiss_icp(dataset)
    ...
    print(f"  evo_rpe kitti {args.out_dir/'poses_gt.txt'} {args.out_dir/'poses_fused.txt'} -a --save_plot {args.out_dir/'fused_rpe.pdf'}")
